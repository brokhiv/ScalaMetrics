...
object FP6:
  ...
  def funcB = (valA: C) => (lambA: C => (List[Int] => List[Int]) => C) => (lambB: Int => Boolean) => 
    lambA(valA)(funcC(_)(lambB))

  def funcC = (a: List[Int]) => (lambA: Int => Boolean) =>
    a.filter(lambA(_))
  ...
  @main def fp6_main(): Unit =
    ...
    val lambA: C => (List[Int] => List[Int]) => C =
      a => lambB =>
      a.map(lambB(_))
