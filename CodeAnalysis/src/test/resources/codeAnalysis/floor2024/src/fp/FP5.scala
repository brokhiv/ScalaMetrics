...
object FP5:
  ...
  case class Graph(nodes: List[Node], edges: List[Edge]):
    lazy val adjacencyList: Map[Node, List[Edge]] = nodes
      .map(node => (node, edges.filter(_.source == node)))
      .toMap
  end Graph

  def funcC: Node => Int => ((Map[Node, Int], Map[Node, Node]), Edge) => (Map[Node, Int], Map[Node, Node]) =
    nodeA => a => { case ((mapA, mapNode), Edge(_, dest, weight)) =>
    val varA = mapA.getOrElse(dest, Int.MaxValue)
    val varB = a + weight
    if varB < varA then (mapA.updated(dest, varB), mapNode.updated(dest, nodeA))
    else (mapA, mapNode)
  }

  def funcD: Node => Node => Map[Node, Node] => List[Node] =
    nodeA => nodeB => mapNode => {
    if nodeA == nodeB then List(nodeA)
    else mapNode.get(nodeB) match
      case Some(node) => nodeB :: funcD(nodeA)(node)(mapNode)
      case None => Nil
  }

  def funcA: Graph => Node => Node => Option[(Int, List[Node])] =
    graph => nodeA => nodeB => {
      @tailrec
      def funcB(mapA: Map[Node, Int], mapNode: Map[Node, Node], adjacency: Map[Node, List[Edge]]): (Map[Node, Int], Map[Node, Node]) =
        if mapA.isEmpty then (mapA, mapNode)
        else
