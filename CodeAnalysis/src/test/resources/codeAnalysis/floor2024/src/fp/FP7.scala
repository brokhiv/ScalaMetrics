...
object FP7 {
  ...
  def funcA = (a: String) => (b: String) => funcD(a, b, 0, 0, funcB(b), Nil)

  def funcB: String => Array[Int] = a => {
    @tailrec
    def funcC(b: Int, c: Int, d: Array[Int]): Array[Int] = {
      if (b == a.length) d
      else if (a(b) == a(c)) funcC(b + 1, 0, d.updated(b, c + 1))
      else if (c != 0) funcC(b, d(c - 1), d)
  ...
  def funcD(a: String, b: String, c: Int, d: Int, e: Array[Int], result: List[Int]): List[Int] = {
    if (c == a.length) result
    else if (a(c) == b(d)) {
      if (d == b.length - 1) funcD(a, b, c + 1, d + 1, e, result.appended(c - d))
      else funcD(a, b, c + 1, d, e, result)
    }
    else if (d != 0) funcD(a, b, c, e(d - 1), e, result)
    else funcD(a, b, c + 1, d, e, result)
  }
