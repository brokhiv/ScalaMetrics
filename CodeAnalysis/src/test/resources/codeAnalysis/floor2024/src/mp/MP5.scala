      ...
      val (listA, mapA) = nodes.foldLeft(
        ...
        if (listB.isEmpty || listB.last != nodeB) {
          ...
          if (pairBnodeB.isDefined &&
            ...
            val listE = if (!currentNode.map(_._1).contains(nodeA)) listB :+ currentNode.get._1
            else List(currentNode.get._1)
            (listE, mapB_)
          }
        }
        else (listB, mapB)
      }
      if (listA.nonEmpty && listA.last == nodeB) mapA.get(nodeB)
      else None
    }
    ...
    private def funcB(
     ...
     listNode: List[Node]
      ...
      listEdge.foldLeft(mutable.Map.from(mapA)) { (mapB, edge) =>
        ...
        if (varA < varB) mapB.put(edge.dest, (varA, listNode :+ edge.dest))
        mapB
      }.toMap
    }
  }
